...
# ========== THE "PRO VOID" ENVIRONMENT ==========
# A sleek, dark reflective floor with a subtle cyber-grid
floor = Entity(model='plane', scale=100, color=color.gray(0.05))
grid = Entity(model='grid', scale=100, color=color.cyan, position=(0, 0.01, 0), rotation_x=90)
grid.alpha = 0.1 # Keep it subtle

# Add fog to hide the edges of the world seamlessly
scene.fog_density = 0.03
scene.fog_color = color.black

# ========== LIGHTING ==========
sun = DirectionalLight(y=4, z=2, shadows=True, color=color.white)
sun.look_at(Vec3(0, 0, 0))
AmbientLight(color=color.rgba(100, 100, 120, 0.4)) # Cool blueish ambient light
