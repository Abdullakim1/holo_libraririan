...
# ========== HOLOGRAM SHADER (only for rings, NOT character) ==========
holo_shader = Shader(
    fragment='''
    #version 140
...
# ========== LOAD ANIME CHARACTER (NO SHADER - keeps original colors) ==========
...
character = Entity(
    ...
    rotation_y=180,
    scale=1.5
    # NO shader - keeps original model colors!
)
...
recognizer.pause_threshold = 0.8  # Shorter pause before considering speech done
...
def listen_and_respond():
    ...
    is_listening = True
    status_text.text = "Listening... (speak clearly)"
    print("\nListening...")
        ...
        try:
            with microphone as source:
                ...
                try:
                    ...
                    print(f"  Google heard: {user_text}")
                except:
                    pass
                
                # If Google failed, try Sphinx (offline)
                if not user_text:
                    try:
                ...
                if not user_text:
                    status_text.text = "Couldn't understand. Please speak clearly."
                ...
                # Send to Ollama with context that this might be misheard
                conversation_history.append({"role": "User", "content": user_text})
                
                # Add hint for Ollama if confidence might be low
                prompt = user_text
                if len(user_text.split()) < 3:  # Very short input might be misheard
                    prompt = f"(This might be misheard speech) {user_text}"
...
def input(key):
    ...
    if key == 'space' and not input_active and not is_talking:
        ...
        input_active = True
        keyboard_text = ""
        status_text.text = "Type message (Enter to send, Esc to cancel): "
...
if __name__ == '__main__':
    ...
    try:
        test = requests.post(OLLAMA_URL, 
            json={"model": OLLAMA_MODEL, "prompt": "Hi", "stream": False, "options": {"max_tokens": 5}}, 
            timeout=10)
