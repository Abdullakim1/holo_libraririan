...
class Converter():
    ...
    def load_primitive(self, geom_node, gltf_primitive, gltf_mesh, gltf_data):
        ...
        accessors = sorted(accessors, key=lambda x: x.get('bufferView', -1))
        data_copies = []
        ...
        for buffview, accs in itertools.groupby(accessors, key=lambda x: x.get('bufferView', -1)):
            buffview = gltf_data['bufferViews'][buffview]
            accs = sorted(accs, key=lambda x: x.get('byteOffset', 0))
            is_interleaved = len(accs) > 1 and 'byteStride' in buffview and accs[1].get('byteOffset', 0) < buffview['byteStride']
