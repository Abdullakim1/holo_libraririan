...
class VisionSystem:
    ...
    def __init__(self):
        ...
        self.known_faces_path = "face_known"
        ...
        # Initialize webcam
        self._init_webcam()
        self.has_scanned_this_visit = False
    ...
    def identify_user(self, frame):
        """Runs facial recognition - locks identity once confirmed"""
        if self.is_identifying:
            print("⏳ identify_user: Already running, skipping")  # DEBUG
            return
        
        print("🔍 identify_user: Starting recognition...")  # DEBUG
            
        self.is_identifying = True
        
        temp_img_path = "temp_face.jpg"
        cv2.imwrite(temp_img_path, frame)
        print(f"📸 Saved frame to {temp_img_path}")  # DEBUG
        
        def run_external_recognition():
            try:
                print("🔄 Running recognize.py...")  # DEBUG
                result = subprocess.run(
                    [sys.executable, 'recognize.py', temp_img_path, self.known_faces_path], 
                    capture_output=True, 
                    text=True,
                    timeout=30
                )
                ...
                print(f"DEBUG recognize.py output: {output}")
                ...
                for line in output.split('\n'):
                    if line.startswith("MATCH:"):
                        ...
                        # 🔒 LOCK LOGIC
                        if self.current_user_name in ["Unknown", "Guest"]:
                            if name != "Guest":
                                self.current_user_name = name
                                print(f"🆔 Identity Confirmed: {self.current_user_name} 🔒")
                                if self.on_person_change_callback:
                                    self.on_person_change_callback(self.person_type, self.person_type)
                        elif name != "Guest" and name != self.current_user_name:
                            print(f"🔄 New person detected: {name} (was {self.current_user_name})")
                            self.current_user_name = name
                            print(f"🆔 Identity Updated: {self.current_user_name}")
                ...
                self.is_identifying = False
                print("✅ Recognition complete")  # DEBUG
                
        threading.Thread(target=run_external_recognition, daemon=True).start()

    def get_detected_name(self):
    ...
    def _analyze_face(self, frame, gray):
            ...
            if len(faces) == 0:
                if self.person_present and (time.time() - self.last_seen_timer > 5.0):
                    self.person_present = False
                    self.person_detected = False
                    old_type = self.person_type
                    self.person_type = "unknown"
                    
                    # 🔥 Reset identity and session when person leaves!
                    self.current_user_name = "Unknown"
                    self.has_scanned_this_visit = False  # Ready for the next person
                    
                    print("👋 Person left - identity and session reset")
                    if self.on_person_change_callback:
                        self.on_person_change_callback(old_type, "unknown")
                return

            # If faces ARE found:
            was_absent = not self.person_present  
            self.person_present = True
            self.person_detected = True
            self.last_seen_timer = time.time()
            
            if was_absent:
                self.has_scanned_this_visit = False # They just walked up
            ...
            # 🔥 THE CRITICAL FIX: "One Scan Per Visit"
            # Only run recognition if we haven't scanned them yet during this visit!
            if not self.is_identifying and not self.has_scanned_this_visit:
                print("🔍 Running face recognition (Scanning new visitor...)")
                self.has_scanned_this_visit = True # Lock it so it doesn't run again!
                self.identify_user(frame)
            
            # Estimate age and gender using Caffe models
